...
co.shp <- readOGR(dsn="co_occ_shp/", layer = "co_occ") %>% 
  spTransform(.,
              CRSobj = CRS(base.proj))
...
past.climna <- raster::stack("/Users/DanielPerret/Box/01. daniel.perret Workspace/PROJECTS/PILA/climNA_rasters/MAT.tif",
                             "/Users/DanielPerret/Box/01. daniel.perret Workspace/PROJECTS/PILA/climNA_rasters/MAP.tif",
                             "/Users/DanielPerret/Box/01. daniel.perret Workspace/PROJECTS/PILA/climNA_rasters/CMD.tif")%>% 
...
dall.er.ablapien <- dall.er.sp %>%
  ...
  mutate(quad.baa = case_when(CHNG_PERC.bah_19 < 0 & CHNG_PERC.bah_93 < 0 ~ 1,
                              CHNG_PERC.bah_19 < 0 & CHNG_PERC.bah_93 >= 0 ~ 2,
                              CHNG_PERC.bah_19 >= 0 & CHNG_PERC.bah_93 >= 0 ~ 3,
                              CHNG_PERC.bah_19 >= 0 & CHNG_PERC.bah_93 < 0 ~ 4),
         quad.19 = case_when(CHNG_PERC.tph_19 < 0 & CHNG_PERC.bah_19 < 0 ~ "decline",
                             CHNG_PERC.tph_19 < 0 & CHNG_PERC.bah_19 >= 0 ~ "development",
                             CHNG_PERC.tph_19 >= 0 & CHNG_PERC.bah_19 >= 0 ~ "densification",
                             CHNG_PERC.tph_19 >= 0 & CHNG_PERC.bah_19 < 0 ~ "turnover"),
         quad.93 = case_when(CHNG_PERC.tph_93 < 0 & CHNG_PERC.bah_93 < 0 ~ "decline",
                             CHNG_PERC.tph_93 < 0 & CHNG_PERC.bah_93 >= 0 ~ "development",
                             CHNG_PERC.tph_93 >= 0 & CHNG_PERC.bah_93 >= 0 ~ "densification",
                             CHNG_PERC.tph_93 >= 0 & CHNG_PERC.bah_93 < 0 ~ "turnover"),
         mult.comp.coexist = case_when(quad.19 == "decline" & quad.93 == "decline" ~ "general decline",
                                       quad.19 == "turnover" & quad.93 == "decline" ~ "general decline",
                                       quad.19 == "decline" & quad.93 == "turnover" ~ "general decline",
                                       
                                       quad.19 == "decline" & quad.93 %in% c("development","densification") ~ "mismatched trajectories",
                                       quad.93 == "decline" & quad.19 %in% c("development","densification") ~ "mismatched trajectories",
                                       
                                       quad.19 == "turnover" & quad.93 %in% c("turnover","development","densification") ~ "vulnerable",
                                       quad.93 == "turnover" & quad.19 %in% c("turnover","development","densification") ~ "vulnerable",
                                       #quad.19 == "turnover" & quad.93 == "densification" ~ "depends",
                                       #quad.93 == "turnover" & quad.19 == "densification" ~ "depends",
                                       #quad.93 == "turnover" & quad.19 == "densification" ~ "depends",
                                       #quad.93 == "turnover" & quad.19 == "densification" ~ "depends",
                                       
                                       quad.19 %in% c("development","densification") & quad.93 %in% c("development","densification") ~ "likely persistence"))
...
ggplot() +
  ...
  scale_fill_discrete(
    ...
    #type=c("dodgerblue4","firebrick2","skyblue1","firebrick2")
    type = c("firebrick4","dodgerblue4","skyblue1","firebrick2")
  )+
...
all.fia$PLOT <- all.fia$PLOT %>% 
  left_join(dall.er.ablapien %>% 
              sf::st_drop_geometry() %>% 
              select(MAP_UNIT_S,quad.baa, quad.19, quad.93, mult.comp.coexist),
...
mort.src.est %>% 
  ...
  geom_errorbar(aes(ymin = (PREV_TOTAL - (PREV_TOTAL_SE)),
                ...
                width=0.3,
                lwd=0.8)+
  facet_wrap(facets=~factor(mult.comp.coexist, levels=c("likely persistence",
                                                        "vulnerable",
...
ind.mort.dat <- all.fia$TREE %>% 
  ...
  left_join(.,
            all.fia$PLOT %>% 
              ...
              mutate(MAT_anom = MAT_recent-MAT_19802010,
                     MAP_anom = MAP_recent-MAP_19802010,
                     CMD_anom = CMD_recent-CMD_19802010,
